...
void Simulation::run() {
    ...
    auto poisson_solver =
        spark::electromagnetics::DirichletPoissonSolver(parameters_.nx, parameters_.dx);
    ...
    for (step = 0; step < parameters_.n_steps; ++step) {
        ...
        spark::electromagnetics::charge_density(parameters_.particle_weight, ion_density_,
        ...
        spark::interpolate::field_at_particles(electric_field_, electrons_);
        spark::interpolate::field_at_particles(electric_field_, ions_);
...
spark::collisions::MCCReactionSet<1, 3> Simulation::load_electron_collisions() {
    ...
    spark::collisions::ReactionConfig<1, 3> electron_reaction_config{
        ...
        std::make_unique<spark::collisions::StaticUniformTarget<1, 3>>(parameters_.ng, parameters_.tg),
